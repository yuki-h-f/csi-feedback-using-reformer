...
from .transformer.Reformer  import ReformerEncoder

class BsSideReformerReconstructor(torch.nn.Module):
    def __init__(self, num_embeddings: int, max_len: int, pad_idx: int, d_model: int, N: int, d_ff: int, heads_num: int, dropout_rate: float, layer_norm_eps: float, device: torch.device = torch.device("cpu"), code_rate: int = 1) -> None:
        ...
        self.encoder = ReformerEncoder(num_embeddings, max_len, pad_idx, d_model, N, d_ff, heads_num, dropout_rate, layer_norm_eps, device)
